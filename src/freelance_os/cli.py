...
def pull(
    source: Optional[List[str]] = typer.Option(
        None, "--source", help="Source(s) to fetch (remotive, remoteok, jobicy, hn). Default: all."
    ),
    ...
    if requested_sources:
        unknown = [s for s in requested_sources if s not in SOURCES]
        if unknown:
    ...
    if not emit_json:
        src_label = ", ".join(requested_sources or list(SOURCES.keys()))
        console.print(f"[cyan]Fetching leads from:[/cyan] {src_label}")

    leads = fetch_leads(requested_sources)
